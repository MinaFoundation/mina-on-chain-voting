 ...
 use crate::{DuplicateCandidateMode, OverVoteRule, MaxSkippedRank, VoteRules, VotingErrors, VotingResult, Builder, Ballot, BallotChoice, Candidate, RoundStats, EliminationStats, EliminationAlgorithm, TieBreakMode};
 use std::{
 ...
 impl RankedVote {
    ...
    pub fn process_ranked_vote(self, id: usize, tip: i64) -> Wrapper<HashMap<String, RankedVotes>> {
     ...
     fn filtered_candidate(
         ...
         if let Some((first_valid, rest)) = advance_voting(
             ...
             Some(RankedVote { first_valid, rest })
         } else {
             None
         }
     }
 ...
 ///
 /// This interface is potentially faster and less memory intensive than [`run_simple_election`].
 /// It also allows fine-grained error control when validating each vote. If you want a simpler
 /// interface, consider using [`run_simple_election`].
 ///
 ...
 /// # let _ = env_logger::try_init();
 ///
 /// let results = ranked_voting::run_simple_election(&vec![
