use crate::{Ballot, BallotChoice, Candidate, VoteRules, VotingErrors};
...
impl Builder {
  pub fn new(rules: &VoteRules) -> Result<Builder, VotingErrors> {
    Ok(Builder { _rules: rules.clone(), _candidates: None, _votes: Vec::new() })
  }
  ...
  pub fn candidates(self, cands: &[String]) -> Result<Builder, VotingErrors> {
    Ok(Builder {
      _rules: self._rules,
      _candidates: Some(
        cands.to_vec().iter().map(|name| Candidate { name: name.clone(), code: None, excluded: false }).collect(),
      ),
      _votes: Vec::new(),
    })
  }
  ...
  ///
  /// candidates: the list of choices made by the voter, in order. Choices do
  /// not need to be unique, or distinct or non-empty.
  pub fn add_vote(&mut self, candidates: &[Vec<String>], count: u32) -> Result<(), VotingErrors> {
    ...
    self.add_vote_2(&Ballot { count: count as u64, candidates: choices })
  }
